...
# Hasta aca todo segun el metodo modulador

# aplicamos un sistema parecido a un switch, simulando el comportamiento de una tabla de verdad
    ...
    for i in range(0, N, 3):
        ...
        if bits[i] == 0 and bits[i+1] == 0 and bits[i+2] == 0:
            A1 = -h
            A2 = -h
            senal_Tx[i*mpp: (i+1)*mpp] = portadora1*A1 + portadora2*A2      
...
# Ahora encontramos la potencia de la simulacion
    

    W = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)

    return senal_Tx, W, portadora1, portadora2
...
def demodulador8PSK(senal_Rx, portadora_1, portadora_2, mpp):
    ...
    # hasta aca todo como en el metodo demodulador
  ...
  # Pseudo-energía de un período de la portadora
    ...
    for i in range(N):
        ...
        prod_1 = senal_Rx[i*mpp : (i+1)*mpp] * portadora1
        P1 = np.sum(prod_1) 
        
        # Para la primer función
        prod_2 = senal_Rx[i*mpp : (i+1)*mpp] * portadora2
        P2 =np.sum(prod_2)
        
        senal_demodulada[i*mpp : (i+1)*mpp] = prod_1 + prod_2

        # Se define la posicion en el mapa

        if i % 3 == 0:
            
            if -(h+1)*Es1/2 <= P1 <= -h*Es1/2 and -(h+1)*Es2/2 <= P2 <= -h*Es2/2:
                bits_Rx[i] = 0
                bits_Rx[i+1] = 0
                bits_Rx[i+2] = 0
            elif -(h+1)*Es1/2 >= P1 and -h*Es2/2 <= P2 <= h*Es2/2:
                bits_Rx[i] = 0
                bits_Rx[i+1] = 0
                bits_Rx[i+2] = 1
            elif -h*Es1/2 <= P1 <= h*Es1/2 and  P2 >= (1+h)*Es2/2:
                bits_Rx[i] = 0
                bits_Rx[i+1] = 1
                bits_Rx[i+2] = 0
            elif -(h+1)*Es1/2 <= P1 <= -h*Es1/2 and h*Es2/2 <= P2 <= (1+h)*Es2/2:
                bits_Rx[i] = 0
                bits_Rx[i+1] = 1
                bits_Rx[i+2] = 1
            elif h*Es1/2 >= P1 >= -h*Es1/2 and -(h+1)*Es2/2 >= P2:
                bits_Rx[i] = 1
                bits_Rx[i+1] = 0
                bits_Rx[i+2] = 0
            elif h*Es1/2 <= P1 <= (h+1)*Es1/2 and -(h+1)*Es2/2 <= P2 <= -h*Es2/2:
                bits_Rx[i] = 1
                bits_Rx[i+1] = 0
                bits_Rx[i+2] = 1
            elif h*Es1/2 <= P1 <= (1+h)*Es1/2 and h*Es2/2 <= P2 <= (1+h)*Es2/2:
                bits_Rx[i] = 1
                bits_Rx[i+1] = 1
                bits_Rx[i+2] = 0
            elif P1 >= (1+h)*Es1/2 and h*Es2/2 >= P2 >= -h*Es2/2:
                bits_Rx[i] = 1
                bits_Rx[i+1] = 1
                bits_Rx[i+2] = 1

                
    # devolvemos la señal demodulada    
    return bits_Rx.astype(int), senal_demodulada
...
senal_Tx, W, portadora1, portadora2 = modulador8PSK(bits_Tx, fc_2, mpp_2)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, W, SNR_2)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador8PSK(senal_Rx, portadora1, portadora2, mpp)
...
va1 = stats.bernoulli(0.5)
va2 = stats.bernoulli(0.5) 

# Número de realizaciones
realizaciones = 10000

# Funciones del tiempo S(t) con N cantidad de realizaciones
S_t = np.empty((realizaciones, len(t2))) 
...
for i in range(realizaciones):
    ...
    s_t = A1 * np.cos(2*np.pi*fc*t2) + A2 * np.sin(2*np.pi*fc*t2)
    S_t[i,:] = s_t
        
    plt.plot(t2, s_t)
    
# Promedio de proceso 
P = [np.mean(S_t[i,:]) for i in range(len(t2))]
plt.plot(t2, P, lw=4, label='Valor esperado teórico')
    
# Promedio de Tx
P = [np.mean(senal_Tx) for i in range(len(t2))]
plt.plot(t2, P, ':', lw=4, label='Valor esperado de la señal modulada')
    
# Realizaciones, su promedio calculado y su promedio teórico
